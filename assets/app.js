...
(() => {
  ...
  // Thumbnails (optional)
  const rendered = ["lehrer_p03.png", "lehrer_p04.png", "lehrer_p06.png", "lehrer_p07.png", "lehrer_p08.png", "lehrer_p09.png", "lehrer_p11.png", "lehrer_p14.png", "lehrer_p15.png", "lehrer_p16.png", "lehrer_p17.png", "lehrer_p19.png", "lehrer_p20.png", "lehrer_p21.png", "lehrer_p22.png", "lehrer_p23.png"];
  if (rendered.length){
    rendered.forEach(name => {
      ...
      img.alt = name;
      img.title = "Abbildung öffnen: " + name;
      img.addEventListener("click", () => {
        const w = window.open("", "_blank");
        w.document.write(`<title>${name}</title><img src="assets/images/${name}" style="max-width:100%;height:auto;margin:0;display:block">`);
      });
      thumbs.appendChild(img);
    });
  } else {
    thumbs.innerHTML = `<div class="small">Keine Vorschaubilder gerendert (Browser‑/System‑Abhängigkeit). Du kannst die PDFs parallel öffnen.</div>`;
  }
  ...
  function roughlyEquals(user, target){
    ...
    if(!u || !t) return false;
    if(u === t) return true;
    // tolerate small typos for short answers
    const maxLen = Math.max(u.length, t.length);
    if(maxLen <= 18) {
      return levenshtein(u,t) <= 2;
    }
    // containment for longer strings
  ...
  function containsAllKeywords(user, keywordGroups){
    const u = norm(user);
    if(!u) return false;
    // each group: user must contain at least one keyword in group
  ...
  function checkAnswer(q, userValue){
    ...
    if(q.type === "textarea_keywords") {
      // accept if at least 3 of groups satisfied (or all if <=3)
    ...
    if(q.type === "tableFill") {
      // userValue is array of lines; require each row roughly matches expected
      const expected = q.rows.map(r => r[1]);
      if(!Array.isArray(userValue) || userValue.length !== expected.length) return false;
      let ok = 0;
      for(let i=0;i<expected.length;i++) {
        if(roughlyEquals(userValue[i], expected[i]) || norm(userValue[i]).includes(norm(expected[i]).slice(0, Math.min(40, norm(expected[i]).length)))) ok++;
      }
    ...
    if(q.type === "matchSelect") {
      // userValue: mapping statement index -> chosen group
    ...
    if(q.type === "clozeMC") {
      // userValue array of answers for items
      ...
      for(let i=0;i<q.items.length;i++) {
        ...
        for (const a of acc){
          if(Array.isArray(a)) {
            // require all in array appear
            ok = containsAllKeywords(u, a);
          } else {
            ok = roughlyEquals(u, a);
          }
          if(ok) break;
        }
        if(!ok) return false;
  ...
  function isSolved(qid){
    return !!(state.solved && state.solved[qid]);
  }
  ...
  function build() {
    ...
    DATA.sections.forEach(section => {
      ...
      head.innerHTML = `
        <div>
          <h2>${section.title}</h2>
          <div class="small">${section.ref} · ${section.intro}</div>
        </div>
        <div class="meta">
          <span class="badge">${section.questions.length} Aufgaben</span>
          <span class="badge">Synonyme aktiv</span>
          <span class="badge">Auto‑Lösung nach 3× falsch</span>
        </div>
      `;
      sec.appendChild(head);

      section.questions.forEach((q, idx) => {
        const qEl = renderQuestion(section, q, idx);
        sec.appendChild(qEl);
      });
  ...
  function renderQuestion(section, q, idx){
    ...
    // Restore prior answers (optional)
    state.answers = state.answers || {};
    if(state.answers[q.id]) {
      try {
        if(q.type === "shorttext" || q.type === "textarea_keywords") setUserValue(state.answers[q.id]);
        // for others: ignore to keep simple (can be added later)
      } catch(e) {}
    }
    ...
    function showSolution(force=false){
      const pre = solution.querySelector("pre");
      pre.textContent = q.solution || "";
      if(force || teacherMode) solution.classList.add("show");
    }

    // Always prepare solution text
    showSolution(false);
    ...
    btnCheck.addEventListener("click", () => {
      ...
      // store answer for simple types
      if(q.type === "shorttext" || q.type === "textarea_keywords") {
      ...
      if(a === 1) {
        ...
        showFeedback("hint", `✗ Noch nicht. <strong>Hinweis:</strong> ${q.hint || "Achte auf zentrale Stichworte aus dem Abschnitt."}`);
      } else {
  ...
  function refreshActiveNav(){
    const sections = DATA.sections.map(s=>s.id);
